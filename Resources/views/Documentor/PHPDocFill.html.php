...
class DocBlock {
    ...
    static function AJAX_Scan() {
        $list = RecursiveScanDir(dirname($_SERVER['DOCUMENT_ROOT']));
    ...
    static function AJAX_Analyze() {
        // Which file?
        $file = dirname($_SERVER['DOCUMENT_ROOT'])."/".$_POST['File'];
...
function RecursiveScanDir($dir, $prefix = '') {
    ...
    foreach (scandir($dir) as $f) {
        if ($f !== '.' and $f !== '..' and $f !== 'cache' and $f !== 'logs' and $f !== 'vendor') {
            if (is_dir("$dir/$f")) {
...
?>
    ...
    <style>
        ...
        div#documentor #generator {
            border: 2px solid #DDDDFF;
            margin-left: 200px;
            padding: 10px;
        }

        div#documentor #filelist {
            /* margin-top: 10px; */
        }
        ...
        div#documentor .valid { font-weight: bold; color: green; }
        div#documentor .todo { font-weight: bold; color: red; }
        div#documentor .bold { font-weight: bold !important; }
        div#documentor li { margin-bottom: 15px; }
        div#documentor #blocklist td {
            font-size: 12px;
            border-right: 1px solid #F2F2F2;
            padding-left: 2px;
            font-family: Oxygen, Verdana;
        }
        div#documentor #blocklist th {
            background: #EEE;
        ...
        div#documentor .odd { background: #F0F0F0; }
        div#documentor .dbmissing { background: #FFF0F0; }

        div#documentor label, input { display:block; }
        div#documentor textarea { width: 95%; height: 80px; }
        div#documentor input.text { margin-bottom:12px; width:95%; padding: .4em; }
        div#documentor fieldset { padding:0; border:0; margin-top:25px; }
        div#documentor .ui-dialog .ui-state-error { padding: .3em; }
        div#documentor .validateTips { border: 1px solid transparent; padding: 0.3em; }
        div#documentor .inline { display: inline; }
        div#documentor #menu a { text-decoration: none; }
